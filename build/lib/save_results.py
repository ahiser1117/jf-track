...
def save_multi_object_tracking_to_zarr(
    tracking_results: dict[str, TrackingData],
    output_path: str,
    fps: float,
    parameters: TrackingParameters | None = None,
):
    """
    Save multi-object tracking data to zarr format.
    
    Supports any number of object types (mouth, gonads, tentacle bulbs, etc.).
    
    Args:
        tracking_results: Dictionary mapping object types to TrackingData objects
        output_path: Path to save zarr store
        fps: Video frame rate
        parameters: Optional TrackingParameters to save with results
    """
    print(f"Saving multi-object tracking results to {output_path}...")
    
    # Get frame count from first object type
    if not tracking_results:
        raise ValueError("No tracking results to save")
    
    first_obj_type = list(tracking_results.keys())[0]
    n_frames = tracking_results[first_obj_type].n_frames
    ...
    root.attrs['fps'] = fps
    root.attrs['n_frames'] = n_frames
    root.attrs['object_types'] = list(tracking_results.keys())
    ...
    # Save each object type's tracking data
    for obj_type, tracking_data in tracking_results.items():
        print(f"Saving {obj_type} tracking data...")
        
        # Create group for this object type
        obj_group = root.create_group(obj_type)
        obj_group.attrs['n_tracks'] = tracking_data.n_tracks
        obj_group.attrs['n_frames'] = tracking_data.n_frames
        
        # Track IDs
        track_arr = obj_group.create_array('track', data=tracking_data.track_ids)
        track_arr.attrs['_ARRAY_DIMENSIONS'] = [f'{obj_type}_track']
        
        # Define array creation function for this object type
        def create_obj_array(name, data):
            arr = obj_group.create_array(name, data=data)
            arr.attrs['_ARRAY_DIMENSIONS'] = [f'{obj_type}_track', 'frame']
            return arr
        
        # Save all tracking arrays
        create_obj_array('x', tracking_data.x)
        create_obj_array('y', tracking_data.y)
        create_obj_array('area', tracking_data.area)
        create_obj_array('major_axis_length_mm', tracking_data.major_axis_length_mm)
        create_obj_array('bbox_min_row', tracking_data.bbox_min_row)
        create_obj_array('bbox_min_col', tracking_data.bbox_min_col)
        create_obj_array('bbox_max_row', tracking_data.bbox_max_row)
        create_obj_array('bbox_max_col', tracking_data.bbox_max_col)
        create_obj_array('frame', tracking_data.frame)
    
    # Print summary
    total_tracks = sum(data.n_tracks for data in tracking_results.values())
    obj_summary = ', '.join([f"{obj_type} ({data.n_tracks})" for obj_type, data in tracking_results.items()])
    
    print(f"Saved {total_tracks} total tracks across {len(tracking_results)} object types: {obj_summary}")
    print(f"Total frames: {n_frames}")
    if parameters is not None:
        print("Tracking parameters saved to zarr")


def load_multi_object_tracking_from_zarr(zarr_path: str) -> dict[str, TrackingData]:
    """
    Load multi-object tracking data from zarr format.
    
    Args:
        zarr_path: Path to zarr store
    
    Returns:
        Dictionary mapping object types to TrackingData objects
    """
    print(f"Loading multi-object tracking results from {zarr_path}...")
    
    root = zarr.open_group(zarr_path, mode='r')
    
    # Get object types
    object_types = root.attrs.get('object_types', [])
    if not object_types:
        raise ValueError("No object types found in zarr file")
    
    tracking_results = {}
    
    for obj_type in object_types:
        print(f"Loading {obj_type} tracking data...")
        
        obj_group = root[obj_type]
        n_tracks = obj_group.attrs['n_tracks']
        n_frames = obj_group.attrs['n_frames']
        
        # Load tracking data
        track_ids = obj_group['track'][:]
        x = obj_group['x'][:]
        y = obj_group['y'][:]
        area = obj_group['area'][:]
        major_axis_length_mm = obj_group['major_axis_length_mm'][:]
        bbox_min_row = obj_group['bbox_min_row'][:]
        bbox_min_col = obj_group['bbox_min_col'][:]
        bbox_max_row = obj_group['bbox_max_row'][:]
        bbox_max_col = obj_group['bbox_max_col'][:]
        frame = obj_group['frame'][:]
        ...
        tracking_results[obj_type] = tracking_data
    
    total_tracks = sum(data.n_tracks for data in tracking_results.values())
    obj_summary = ', '.join([f"{obj_type} ({data.n_tracks})" for obj_type, data in tracking_results.items()])
    
    print(f"Loaded {total_tracks} total tracks across {len(tracking_results)} object types: {obj_summary}")
